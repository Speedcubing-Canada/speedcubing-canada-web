...
export const Competitions = () => {
  ...
  const displayedComps = Object.keys(data)
    .filter((key) => {
      ...
      return (
        ...
        (selectedRegions.length === 0 ||
          provinces.some(
            (provinceItem) =>
              provinceItem.label === province &&
              selectedRegions.includes(provinceItem.region_id),
          ))
      );
    })
    .map((key) => data[key]);
  ...
  const regions = Array.from(
    new Set(provinces.map((province) => province.region_id)),
  );
            ...
            <FormControl sx={{ minWidth: 200, textAlign: "left" }}>
              ...
              <Select
                ...
                renderValue={(selected: any[]) => {
                  const translatedRegions = selected.map((region) =>
                    t(`regions.${region}`),
                  );
